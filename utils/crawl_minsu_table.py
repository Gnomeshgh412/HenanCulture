...
# 输出 txt 的目录
TEXT_DIR = Path("D:\\桌面\\New Life\\河南地方志中的民俗要素挖掘与知识图谱构建\\Data\\luoyang_city_gazetteer\\raw_data\\商贸习俗\\04_行会组织")

# 输出图片的根目录（每个页面再建一个子目录）
IMAGE_DIR = Path("D:\\桌面\\New Life\\河南地方志中的民俗要素挖掘与知识图谱构建\\Data\\luoyang_city_gazetteer\\raw_data\\商贸习俗\\04_行会组织\\image")
...
def table_to_text(table: Tag) -> str:
    """
    将 <table> 转成按行、列组织的纯文本。
    每行用 '\t' 分隔单元格，行与行之间用 '\n' 分隔。
    """
    rows = []
    # 只取这一层 table 里的 tr，不跨到别的 table 去
    for tr in table.find_all("tr", recursive=True):
        # 如果这个 tr 里面又嵌了 table，就跳过这个 tr，
        # 真正的数据会在内层 table 里处理
        if tr.find("table"):
            continue
...
def extract_text(content_tag) -> str:
    """
    从正文容器中提取文本：
      - 明确按块级标签 / 换行标签控制换行，保持段落结构
      - table：
          * 如果内部还有 table → 视为布局容器，只递归里面内容
          * 如果是最内层 table → 调用 table_to_text，只输出一次
    """
    pieces: list[str] = []
    seen_table_texts: set[str] = set()  # 防止完全相同的表格重复输出
    ...
    def ensure_newlines(n: int = 1):
        """确保末尾至少有 n 个换行"""
        current = "".join(pieces)
        needed = "\n" * n
        if current.endswith(needed):
            return
        # 去掉末尾杂空格
        while pieces and pieces[-1].isspace() and pieces[-1] != "\n":
            pieces.pop()
        pieces.append(needed)

    def is_layout_table(tag: Tag) -> bool:
        """判断是不是外层布局 table：内部还有 table 就认为是布局。"""
        return bool(tag.find("table"))
    ...
    def walk(node):
        for child in getattr(node, "children", []):
            if isinstance(child, NavigableString):
                ...
                # 1) table 处理：区分布局 table 和 数据 table
                if name == "table":
                    if is_layout_table(child):
                        # 外层布局 table：递归进去，让内层 table 自己处理
                        walk(child)
                    else:
                        # 最内层数据表：只输出一次结构化内容
                        table_txt = table_to_text(child)
                        if table_txt and table_txt not in seen_table_texts:
                            seen_table_texts.add(table_txt)
                            ensure_newlines(2)
                            pieces.append(table_txt)
                            ensure_newlines(2)
                    # 不再向下递归（避免重复）
                    continue
